...
const App = () => {
	...
	return (
		...
		<textarea key="invalid-text" className="w-full bg-red-300" value={lines} onChange={e => setLines(e.target.value)}/>
		</div>
		</main>
	)
	...
	let moves = tree.children?.map(child => moveToCoords(position, child.value));

	const options =  {
		arrows: moves?.map(m => ({ ...m, color: m.isBlunder ? "red" : "green"})) ?? [],
		position,
	}
	...
	return (
		...
		options={options}
		/>
		<textarea key="valid-text" className="w-full" value={lines} onChange={e => setLines(e.target.value)}/>
		</div>
		<div className="flex w-full gap-3">
		{
			tree.children?.map((c, i) => 		
			   <button key={i} className="border-4 rounded p-2 basis-0 grow max-w-xs" onClick={() => onMoveClicked(i)} >{c.value}</button>
		  )
		}
		{history.length > 1 && <button className="border-4 p-2 basis-0 grow max-w-xs" onClick={onBack} >Back</button>}
		</div>
		</main>
	)
}
